...
var KTAppLayoutBuilder = function() {
	...
	var resetButton;
	var sitemapModal;
	var sitemapModalToggle;

	var handleSitemap = function() {	
		if (sitemapModal === null || sitemapModalToggle === null) {
			return;
		}	

		if ( KTCookie.get("sitemap_modal_displayed") !== "1" ) {
			setTimeout(function() {
				const modal = new bootstrap.Modal(sitemapModal);
				modal.show();
	
				const date = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days from now
				KTCookie.set("sitemap_modal_displayed", "1", {expires: date});
			}, 3000);
		} else if ( KTCookie.get("sitemap_modal_popover_displayed") !== "1" ) {
			setTimeout(function() {
				const date = new Date(Date.now() + 10 * 24 * 60 * 60 * 1000); // 10 days from now
				KTCookie.set("sitemap_modal_popover_displayed", "1", {expires: date});
				var popover = bootstrap.Popover.getInstance(sitemapModalToggle);
				popover.show();
			}, 3000);
		}

		const popover = bootstrap.Popover.getInstance(sitemapModalToggle);
		sitemapModalToggle.addEventListener("click", function (e) {
			popover.hide();
		});
	}

	var handleSitemapViewMenu = function() {
		const selected = sitemapModal.querySelector('[data-kt-element="selected"]');
		const selectedTitle = sitemapModal.querySelector('[data-kt-element="title"]');
		const menu = sitemapModal.querySelector('[data-kt-menu="true"]');

		// Toggle Handler
		KTUtil.on(sitemapModal, '[data-kt-mode]', 'click', function (e) {
			const title = this.innerText;	
			const mode = this.getAttribute("data-kt-mode");
			const selectedLink = menu.querySelector('.menu-link.active');
			const viewImage = document.querySelector('#kt_engage_sitemap_view_image');
			const viewText = document.querySelector('#kt_engage_sitemap_view_text');
	...
	return {
		// Public functions
		init: function() {
			sitemapModal = document.querySelector('#kt_engage_sitemap_modal');
			sitemapModalToggle = document.querySelector('#kt_engage_sitemap_toggle');	

			if ( sitemapModal ) {
				handleSitemap();
				handleSitemapViewMenu();
			}
